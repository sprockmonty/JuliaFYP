...
%2nd stage
h2   = rowNorm(r2) - Re;                  %altitude
rho2 = rho0*exp(-h2/h0);               %atmospheric density
D2   = -0.5*CD*Aref*rho2.*rowNorm(v2).*v2;   %drag force vector


%1st stage
rdot1 = v1;
vdot1 = -mu*r1./(rowNorm(r1)).^3 + T1./m1 + D1./m1;
mdot1 = -rowNorm(T1)/(g0*Isp);
%2nd stage
rdot2 = v2;
vdot2 = -mu*r2./(rowNorm(r2)).^3 + T2./m2 + D2./m2;
